#include "Core.h"
        ...
        void Core:: parser(vector<string> program, int core, vector<int>& memory){
            int index;
            ...
            for(auto line : program){
                ...
                encodeInstToMem(div, core, memory);
                //pc+=4;
            }
        }
        ...
        void Core:: setMemory(int wordAddress, int value, vector<int>& memory) {  
            memory[wordAddress] = value;
        }

        void Core:: encodeInstToMem(int arr[], int core, vector<int>& memory){
            stringstream ss;
            ...
            if(core == 0){
                static int index0 = 0;
                setMemory(index0, encoded, memory);
                index0++;
            }
            else if(core == 1){
                static int index1 = 1024;
                setMemory(index1, encoded, memory);
                index1++;
            }
            // cout<<ss.str()<<endl;
            // cout<<encoded<<endl;
        ...
        pair<int, int> Core::  execute(int core, vector<int>& memory){
            int InstBase;                   // memory address
            ofstream outFile1, outFile2;
            if(core == 0){
                InstBase = 0;
                outFile1.open("assets/output1.txt");
                if (!outFile1.is_open()) {
                    cout << "Failed to open output1.txt for writing." << endl;
                    return pair<int,int>();
                }
            }
            else if(core == 1){
                InstBase = 1024;
                outFile2.open("assets/output2.txt");
                if (!outFile2.is_open()) {
                    cout << "Failed to open output2.txt for writing." << endl;
                    return pair<int,int>();
                }
            }
            ...
            do{
                ...
                //cout<<core<<"    "<<opcode<<" "<<rd<<" "<<rs1<<" "<<rs2<<" "<<imm<<endl;
                ...
                switch(opcode){
                    ...
                    case 7: 
                        if(rs2 == 15){              // lw x1,base
                            ...
                            setRegister(rd, getMemory(getRegister(rs1) + (imm/4), memory));     
                        }
                        InstBase++; 
                        (CycInst.second)++;
                        break;
                    case 8: setMemory((getRegister(rs1) + (imm/4)), getRegister(rd), memory);   InstBase++; (CycInst.second)++; break;      // rd and rs1 reverse for sw
                    case 9:
                ...
                if(core == 0){
                    outFile1<<"Cycle: "<<CycInst.first<<endl;
                    for (int j = 0; j < 32; j++) {
                        string reg = "x" + to_string(j);
                        outFile1 << registers[reg] << " ";
                    }
                    outFile1 << endl;
                    ...
                    outFile2<<"Cycle: "<<CycInst.first<<endl;
                    for (int j = 0; j < 32; j++) {
                        string reg = "x" + to_string(j);
                        outFile2 << registers[reg] << " ";
                    }
                    outFile2 << endl;
                    for(int i=3072;i<3094;i++){
                        outFile2<<i<<" "<<memory[i]<<"   ";
                    }
                    outFile2<<endl<<endl;
                }
        ...
        int Core:: getMemory(int wordAddress, vector<int>& memory) {
